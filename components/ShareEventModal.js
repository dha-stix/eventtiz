...
const ShareEventModal = ({ event, closeModal }) => {
	...
	const registerUrl = event.data
		? `/register/${event.id}/${event.data.slug}`
		: `/register/${router.query.id}/${event.slug}`;
	...
	return (
		<div className='w-full h-[100vh] dim absolute top-0 left-0 flex items-center justify-center p-4  z-40'>
			<div className='md:w-[500px] w-full bg-white h-[300px] flex items-center justify-center flex-col rounded-md shadow-[#FFD95A] shadow-md space-y-4 relative'>
				...
				<div className='flex md:flex-row flex-col items-center space-x-3 p-3'>
					<p className='p-2 border-[1px] md:mb-0 mb-4 rounded-md text-gray-400'>
						{registerUrl}
					</p>
					{copied ? (
